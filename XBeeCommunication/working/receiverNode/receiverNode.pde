    ...
    if( xbee802.available() > 0 ) 
    {
      ...
      if( !xbee802.error_RX ) 
      {
        ...
        while( xbee802.pos > 0  )
        {
          ...
          if( strstr(xbee802.packet_finished[xbee802.pos - 1]->data, "END") != NULL)
          {
            USB.println("Closing frame found");
            received = 1;
          }
          else {
	    for( int j=0 ; j < xbee802.packet_finished[xbee802.pos - 1]->data_length ; j++)          
            {           
              buffer[j] = xbee802.packet_finished[xbee802.pos - 1]->data[j];          
            }
          ...
          xbee802.pos = xbee802.pos - 1; //Increment the received packet counter
